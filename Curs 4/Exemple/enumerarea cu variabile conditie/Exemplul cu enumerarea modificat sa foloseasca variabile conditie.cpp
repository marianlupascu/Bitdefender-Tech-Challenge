...
DWORD WINAPI FuncThread(LPVOID param)
{
	while (1)
	{
		...
		if (myQueue.empty())
			SleepConditionVariableCS(&cvSync, &csTasks, INFINITE);
...
int wmain(int argc, wchar_t * argv[])
{
	...
	InitializeConditionVariable(&cvSync);
	...
	WakeAllConditionVariable(&cvSync);
